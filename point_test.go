...
func (s *Ed448Suite) TestDecafDerivePrivateKey(c *C) {
	c.Skip("not having shake")
	...
	priv, _ := curve.decafDerivePrivateKey(sym)

	pub := []uint8{
		0x03, 0xa7, 0x33, 0x96, 0xae, 0x97, 0x10, 0x1d,
		0x9f, 0xf2, 0x8f, 0x72, 0xec, 0x90, 0x18, 0x4d,
		0x44, 0x9e, 0xcc, 0x28, 0x90, 0xeb, 0xb2, 0x0c,
		0x13, 0x6e, 0x7f, 0xbe, 0x02, 0xec, 0x0f, 0xcc,
		0xac, 0x17, 0x4b, 0x42, 0xde, 0xea, 0x68, 0x31,
		0x55, 0xc3, 0xfb, 0x0e, 0x28, 0x04, 0x3d, 0x57,
		0xf7, 0x94, 0x55, 0x25, 0x85, 0x37, 0xd6, 0x15,
	}

	priv2 := []uint8{
		0x07, 0xf8, 0x08, 0x0b, 0x1a, 0x09, 0x27, 0xba,
		0xbb, 0xc2, 0xbe, 0x56, 0x91, 0x4c, 0x35, 0xba,
		0x16, 0x5d, 0x8e, 0x17, 0xa1, 0x72, 0xf1, 0x88,
		0x2f, 0xea, 0x21, 0x6f, 0x2b, 0xd7, 0x77, 0x4c,
		0x76, 0x52, 0x18, 0xab, 0xcc, 0x1b, 0xb8, 0xc1,
		0x30, 0xd3, 0x21, 0x2d, 0xb8, 0xc5, 0x58, 0x95,
		0xe7, 0xac, 0x4c, 0x76, 0xab, 0xd7, 0xb8, 0x29,
	}
	...
	c.Assert(priv.symKey(), DeepEquals, sym2)
	c.Assert(priv.secretKey(), DeepEquals, priv2)
	c.Assert(priv.publicKey(), DeepEquals, pub)
}
